...
type ReferralSystem interface {
	ProcessPayments(app *App, scale map[uint32]float64, batchTs string) error
...
type MigrationFunc func(postgresDSN string, dbInstance *sql.DB) error

// New intantiates a referral app
func (a *App) New(viper *viper.Viper, migrationFunc MigrationFunc) error {
	...
	// Run migrations.
	if err := migrationFunc(viper.GetString(env.DATABASE_DSN_HISTORY), a.RS.GetDb()); err != nil {
		return errors.New("running migrations:" + err.Error())
	} else {
...
func (a *App) DbGetMarginTkn() error {
	...
	if err != nil {
		return errors.New("Table expected (generated by trader backend)" + err.Error())
	}
	defer rows.Close()
